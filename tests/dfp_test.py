...
class DfpHeaderHandlerTest(unittest.TestCase):
  ...
  def testSetHeaders(self):
    ...
    self.assertEquals(network_code, soap_header.networkCode)
    self.assertEquals(''.join([app_name,
  ...
  def testGetService_success(self):
    ...
    with mock.patch('suds.client.Client') as mock_client:
      ...
      mock_client.assert_called_once_with(
          ...
          % (version, service), plugins=[mock.ANY])
...
class DataDownloaderTest(unittest.TestCase):
  ...
  def testDownloadPqlResultSetToCsv(self):
    ...
    self.report_downloader.DownloadPqlResultSetToCsv(
  ...
  def testDownloadReportToFile(self):
    ...
    # TODO(user): This test won't work with Python 3 because the module
    #     name changed. Modify this test to work with urllib.request in Py3
