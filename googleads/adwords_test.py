...
class AdWordsHeaderHandlerTest(unittest.TestCase):
  ...
  def testSetHeaders(self):
    ...
    self.assertEquals(ccid, soap_header.clientCustomerId)
    self.assertEquals(dev_token, soap_header.developerToken)
    self.assertEquals(
        ''.join([user_agent, googleads.adwords._AdWordsHeaderHandler._LIB_SIG]),
        soap_header.userAgent)
    self.assertEquals(validate_only, soap_header.validateOnly)
    self.assertEquals(partial_failure, soap_header.partialFailure)
  ...
  def testGetReportDownloadHeaders(self):
    ...
    # Default returnMoneyInMicros value is false.
  ...
  def testGetService_success(self):
    ...
    with mock.patch('suds.client.Client') as mock_client:
      ...
      mock_client.assert_called_once_with(
          ...
          % (namespace, version, service), plugins=[mock.ANY])
...
class ReportDownloaderTest(unittest.TestCase):
  ...
  def testDownloadReport(self):
        ...
        with mock.patch('urllib2.Request') as mock_request:
          ...
          mock_request.assert_called_once_with(
              ('https://adwords.google.com/api/adwords/reportdownload/%s'
               % self.version),
              urllib.urlencode({'__rdxml': serialized_report}), headers)
    ...
    self.assertEquals(content, output_file.getvalue())
    ...
    with mock.patch('urllib2.urlopen') as mock_urlopen:
      with mock.patch('urllib2.Request') as mock_request:
        ...
        mock_request.assert_called_once_with(
            ('https://adwords.google.com/api/adwords/reportdownload/%s'
             % self.version),
            urllib.urlencode({'__fmt': file_format, '__rdquery': query}),
            headers)
        mock_urlopen.assert_called_once_with(mock_request.return_value)
  ...
  def testExtractError_badRequest(self):
    ...
    self.assertEquals(type_, rval.type)
    self.assertEquals(trigger, rval.trigger)
    self.assertEquals(field_path, rval.field_path)
    self.assertEquals(response.code, rval.code)
    self.assertEquals(response, rval.error)
    self.assertEquals(content, rval.content)
    ...
    response.read.return_value = content
    rval = self.report_downloader._ExtractError(response)
    self.assertEquals(None, rval.type)
    self.assertEquals(None, rval.trigger)
    self.assertEquals(None, rval.field_path)
