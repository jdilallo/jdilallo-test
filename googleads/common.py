...
class Pruner(suds.plugin.MessagePlugin):
  """Suds plugin which prunes empty elements from the message.

  This is necessary because suds seems to send empty XML elements (e.g.,
  <elementName />) for complex types which aren't supplied. AdWords and DFP both
  will not accept empty values for their enum complex types.
  """

  def marshalled(self, context):
    """Prunes the SOAP message if the body contains input parameters."""
    if not context.envelope[1].getChildren()[0].isempty():
